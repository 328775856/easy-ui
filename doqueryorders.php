<?php
  ...
  if($conn->connect_error){
	...
	$result = array(); //定义一个数组
	$sql = "select count(*) from orderinfo where name like '%$keywords3%'"; //查询总条数的sql语句
	$rs = $conn->query($sql);
	$row = $rs->fetch_row(); //读取一行数据
	$result["total"] = $row[0]; //取第一列的值，保存到数组中。
	
	$sql = "select * from orderinfo where name like '%$keywords3%' limit 0,10";
	$rs = $conn->query($sql);
